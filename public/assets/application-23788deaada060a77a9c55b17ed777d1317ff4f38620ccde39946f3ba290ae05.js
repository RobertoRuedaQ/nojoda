...
if( $('.lumni-jodit').length > 0){
  ...
  for(i=0;i<$('.lumni-jodit').length;i++){
    ...
    joditelements[i] = new Jodit(targetId)
  ...
  $('.lumnisubmit').on('click',function(){
    ...
    for(i=0;i < $('.lumni-jodit').length;i++){
      ...
      $( targetId.split('lumnifulleditor')[0]).val(joditelements[i].value)
	...
	function validate_numeric_fields(){
	 	for(i=0;i<$('.numericField').length;i++){
	 		...
	 		if ($(tempNumericId.split('numericField')[0]).val() == ''){
				...
				new Intl.NumberFormat().format($(tempNumericId.split('numericField')[0]).val()) +
				postfix
	 		}
...
function setErrroMessage(element,value_text,field_type){
	...
	if(field_type == 'list' || field_type == 'reference'){
		...
		console.log($('#' + element.id + 'numericField'))
...
;
function lumniPost(path,params){
	$.post(path,params)
  .done(function() {
  })
  .fail(function() {
...
/*
            ...
            if (stepDirection === 'forward'){
            	if($('#new_application')[0].checkValidity()){
                $('.spinner').show();
...
function setGlobalSwitches(){
	...
	$(".switcher-input").on('change', function() {
     ...
     hiddenOne.val(togBtn.prop('checked'));
	})
...
;
$(document).on('turbolinks:load',function () {
	if(typeof document.location.toString().split('#') != 'undefined'){
		targetId = document.location.toString().split('#').pop()
		tabId = 'a[href="#' + targetId + '"]'
		if( $(tabId).length > 0 ){
			$(tabId).tab('show')
		}
	}
...
function attributeTypeFieldChange(){
	$('#formattribute_name_').on('change',function(){
